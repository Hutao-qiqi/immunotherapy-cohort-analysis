...
msk_path <- "MSK_NSCLC_logistic_prediction.grouped_by_IMvigor210_cutoffs.8q24_PDL1_TMB.tsv"
stopifnot(file.exists(msk_path))

msk <- read.delim(msk_path, header = TRUE, sep = "\t", check.names = FALSE, stringsAsFactors = FALSE)

msk$BenefitGroup <- factor(msk$BenefitGroup, levels = c("Low", "Inter", "High"))
msk$ResponseBin <- ifelse(msk$binaryResponse1 == 1, "Benefit", "No benefit")
msk$ResponseBin <- factor(msk$ResponseBin, levels = c("No benefit", "Benefit"))

# Continuous association test in MSK: glm(binaryResponse1 ~ pred)
glm_df <- msk[!is.na(msk$binaryResponse1) & !is.na(msk$pred), ]
glm_fit <- tryCatch(
...
subtitle_txt <- paste0(
  "MSK logistic: binaryResponse1 ~ pred; p(pred)=",
...
write.table(
  glm_out,
  file = "MSK_NSCLC_glm_binaryResponse1_on_pred.8q24_PDL1_TMB.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE
...
ct_all <- table(msk$BenefitGroup, msk$ResponseBin)
...
benefit_df$label <- paste0(round(100 * benefit_df$Fraction), "%")
...
p <- ggplot(agg, aes(x = BenefitGroup, y = Fraction, fill = ResponseBin)) +
  ...
  labs(
    title = "Benefit Score (validation: MSK_NSCLC)",
...
ggsave("MSK_NSCLC_BenefitScore_validation_fraction.8q24_PDL1_TMB.png", plot = p, width = 4.2, height = 3.4, dpi = 300)

ggsave("MSK_NSCLC_BenefitScore_validation_fraction.8q24_PDL1_TMB.pdf", plot = p, width = 4.2, height = 3.4)

cat("MSK NSCLC counts by group (Benefit/No benefit), all groups:\n")
print(ct_all)

cat("\nMSK logistic regression: binaryResponse1 ~ pred\n")
if (is.null(glm_fit)) {
  cat("glm failed\n")
} else {
