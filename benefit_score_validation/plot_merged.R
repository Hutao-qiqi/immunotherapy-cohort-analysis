...
in_path <- "ValidationMerged_RIAZ_LIU_MSK_logistic_prediction.grouped_by_IMvigor210_cutoffs.8q24_PDL1_TMB.tsv"
...
# Continuous association test in merged cohort: glm(binaryResponse1 ~ pred)
...
subtitle_txt <- paste0(
  "Merged logistic: binaryResponse1 ~ pred; p(pred)=",
...
glm_out <- data.frame(
  Cohort = "ValidationMerged_RIAZ_LIU_MSK",
...
write.table(
  glm_out,
  file = "ValidationMerged_RIAZ_LIU_MSK_glm_binaryResponse1_on_pred.8q24_PDL1_TMB.tsv",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE,
  col.names = TRUE
...
p <- ggplot(agg, aes(x = BenefitGroup, y = Fraction, fill = ResponseBin)) +
  ...
  labs(
    title = "Benefit Score (validation: RIAZ+LIU+MSK pooled)",
...
ggsave(
  "ValidationMerged_RIAZ_LIU_MSK_BenefitScore_validation_fraction.8q24_PDL1_TMB.png",
  plot = p,
  width = 4.2,
  height = 3.4,
  dpi = 300
...
ggsave(
  "ValidationMerged_RIAZ_LIU_MSK_BenefitScore_validation_fraction.8q24_PDL1_TMB.pdf",
  plot = p,
  width = 4.2,
  height = 3.4
)

cat("ValidationMerged_RIAZ_LIU_MSK counts by group (Benefit/No benefit), all groups:\n")
print(ct_all)

cat("\nValidationMerged_RIAZ_LIU_MSK logistic regression: binaryResponse1 ~ pred\n")
if (is.null(glm_fit)) {
  cat("glm failed\n")
} else {
